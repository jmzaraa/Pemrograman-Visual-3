...
class Ui_FormSupplier(object):
    ...
    def cellClick(self, row, column):
        try:
            ...
            akun_bank = self.tableWidget.item(row, 8).text()
            no_rekening = self.tableWidget.item(row, 9).text()
            ...
            self.lineEdit_namaakunbank.setText(akun_bank)
            self.lineEdit_noakunbank.setText(no_rekening)
    ...
    def insertSupplier(self):
        try:
            ...
            akun_bank = self.lineEdit_namaakunbank.text()
            no_rekening = self.lineEdit_noakunbank.text()

            sql = "INSERT INTO supplier (id, NIK_supp, name, telp_supp, alamat_supp, perusahaan, email_supp, nama_bank, akun_bank, no_rekening) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
            val = (id, nik, name, telp, alamat, perusahaan, email, nama_bank, akun_bank, no_rekening)
    ...
    def updateSupplier(self):
        try:
            ...
            sql = "UPDATE supplier SET NIK_supp = %s, name = %s, telp_supp = %s, alamat_supp = %s, perusahaan = %s, email_supp = %s, nama_bank = %s, akun_bank = %s, no_rekening = %s WHERE id = %s"
            val = (nik, name, telp, email, alamat, perusahaan, nama_bank, akun_bank, no_rekening, id)
            cursor.execute(sql, val)
            mydb.commit()
    ...
    def cariData(self):
        try:
            ...
            cursor = mydb.cursor()
            sql = "SELECT * FROM supplier WHERE id LIKE %s OR NIK_supp LIKE %s OR name LIKE %s OR telp_supp LIKE %s OR email_supp LIKE %s OR alamat_supp LIKE %s OR perusahaan LIKE %s OR nama_bank LIKE %s OR akun_bank LIKE %s OR no_rekening LIKE %s"
